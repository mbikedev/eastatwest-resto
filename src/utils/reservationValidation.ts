...
export class ReservationValidator {
  ...
  static async validate(form: ReservationForm, t: (key: string) => string): Promise<ValidationResult> {
    ...
    // Holiday validation (async)
    await this.validateHoliday(form.date, errors);
  ...
  // Validate holiday - check if selected date is a holiday
  private static async validateHoliday(date: string, errors: ValidationError[]): Promise<void> {
    if (!date) return;

    try {
      const response = await fetch(`/api/holidays/check?date=${date}`);

      if (!response.ok) {
        // Don't block reservation if holiday check fails
        console.error('Holiday check failed:', response.statusText);
        return;
      }
      ...
      if (result.success && result.is_holiday) {
        errors.push({
          field: 'date',
          message: result.message || 'The restaurant is closed on this date.'
        });
      }
    } catch (error) {
      // Don't block reservation if holiday check fails
