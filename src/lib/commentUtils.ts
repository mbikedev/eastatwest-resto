...
/**
 * Fetch all comments from all blog posts (admin)
 */
export const fetchAllComments = async (includeUnapproved = true): Promise<Comment[]> => {
  try {
    let query = supabase
      .from('comments')
      .select('*')
...
/**
 * Disapprove a comment (admin functionality)
 */
export const disapproveComment = async (commentId: string): Promise<{ success: boolean; error?: string }> => {
  try {
    const { error } = await supabase
      .from('comments')
